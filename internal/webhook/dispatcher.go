...
func (d *Dispatcher) deliver(ctx context.Context, event db.GetDueWebhooksRow) {
	...
	if err != nil {
		...
		_ = d.queries.MarkWebhookFailed(ctx, db.MarkWebhookFailedParams{
			ID:            event.ID,
			NextAttemptAt: next,
		})
		return
	...
	request.Header.Set("X-Operator-Key", "demo-operator-key")
	...
	if response.StatusCode >= 200 && response.StatusCode < 300 {
		_ = d.queries.MarkWebhookSuccess(ctx, event.ID)
